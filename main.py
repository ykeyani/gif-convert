...
class App:
    ...
    def import_images(self):
        ...
        for file_path in file_paths:
            try:
                ...
                self._add_frame(Image.open(file_path))
            except EXCEPTION:
                continue
    ...
    def save_images(self):
        ...
        print(self.frames[0].mode)

        self.frames[0].save(output_file_path,
                            format="GIF",
                            append_images=self.frames[1:],
                            save_all=True,
                            duration=fps_to_ms(self.frame_rate.get()),
                            transparency=self.frames[0].info['transparency'],
                            disposal=int(self.disposal.get()[0]),
                            optimize=False,
                            loop=0)

    def _add_frame(self, image):
        # Get the alpha band
        alpha = image.split()[3]
        ...
        image.save('mouse.gif', transparency=255)
